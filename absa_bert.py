...
class ABSAModel():
    ...
    def train(self, data: pd.DataFrame, n_epoch: int, device, batch_size: int=5, lr: float=3*1e-5, load_model: str | None=None) -> None:
        ...
        for epoch in tqdm(range(n_epoch)):
            ...
            for text, y in tqdm(self.train_dataloader, leave=False):
                ...
                tokens = self.tokenizer.batch_encode_plus(text,
                                                          padding='max_length',
                                                          return_tensors='pt')
                logits = self.model(tokens['input_ids'].to(device),
    ...
    def predict(self, dev: pd.DataFrame, load_model: str | None=None) -> tuple[list[int], float]:
        if load_model is not None:
            if os.path.exists(load_model):
                self.load_model(self.model, load_model)
            else:
        ...
        with torch.no_grad():
            for text, y in tqdm(self.test_dataloader):
                tokens = self.tokenizer.batch_encode_plus(text,
                                                        ...
                                                        return_tensors='pt')
                logits = self.model(tokens['input_ids'].to(device),
                                    attention_mask=tokens['attention_mask'].to(device)).argmax(dim=1)

                self.predictions += list(logits.data.cpu().numpy())
                self.actuals += list(y.data.cpu().numpy())
            return self.predictions, accuracy_score(self.actuals, self.predictions)